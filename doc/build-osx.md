...
This guide describes how to build Actiniumd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Actinium from source.
...
### 4. Clone Actinium repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Actinium repository to a directory.
All build scripts and commands will run from this directory.

``` bash
git clone https://github.com/Actinium/Actinium-ng.git
```
...
It is not necessary to build wallet functionality to run `Actiniumd` or  `Actinium-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/Actinium/Actinium-ng/blob/master/doc/descriptors.md), `sqlite` is required.
...
Actinium includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Actinium application using `make deploy`.
...
There are many ways to configure Actinium, here are a few common examples:
...
Run the following in your terminal to compile Actinium:
...
Actinium should now be available at `./src/Actiniumd`.
If you compiled support for the GUI, it should be available at `./src/qt/Actinium-qt`.

The first time you run `Actiniumd` or `Actinium-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Actinium/
```
...
./src/qt/Actinium-qt -server # Starts the Actinium-qt server mode, allows Actinium-cli control
```
