...
static RPCHelpMan setmocktime() {
  return RPCHelpMan{
      ...
      [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
        ...
        // For now, don't change mocktime if we're in the middle of validation,
        // as this could have an effect on mempool time-based eviction, as well
        // as IsCurrentForFeeEstimation() and IsInitialBlockDownload().
        ...
        if (time < 0) {
          throw JSONRPCError(
              ...
              strprintf("Mocktime cannot be negative: %s.", time));
        }
        SetMockTime(time);
...
static RPCHelpMan invokedisallowedsyscall() {
  return RPCHelpMan{
      ...
      "\nInvoke a disallowed syscall to trigger a syscall sandbox violation. "
      "Used for testing purposes.\n",
      {},
      RPCResult{RPCResult::Type::NONE, "", ""},
      RPCExamples{HelpExampleCli("invokedisallowedsyscall", "") +
                  HelpExampleRpc("invokedisallowedsyscall", "")},
      [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
                  ...
                  {RPCResult::Type::OBJ,
                   ...
                   "Information about locked memory manager",
                   {
                       ...
                       {RPCResult::Type::NUM, "locked",
                        ...
                        "number is smaller than total, locking pages failed at "
...
static RPCHelpMan echoipc() {
  return RPCHelpMan{
      "echoipc",
      "\nEcho back the input argument, passing it through a spawned process in "
      "a multiprocess build.\n"
      ...
      [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
        interfaces::Init& local_init =
            *EnsureAnyNodeContext(request.context).init;
...
static RPCHelpMan getindexinfo() {
  return RPCHelpMan{
      ...
      "in the node.\n",
      {
          {"index_name", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG,
      ...
      [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
        ...
        if (g_coin_stats_index) {
          result.pushKVs(
              SummaryToJSON(g_coin_stats_index->GetSummary(), index_name));
        }

        ForEachBlockFilterIndex(
            [&result, &index_name](const BlockFilterIndex& index) {
...
void RegisterNodeRPCCommands(CRPCTable& t) {
  static const CRPCCommand commands[] {
    {"control", &getmemoryinfo}, {"control", &logging}, {"util", &getindexinfo},
        {"hidden", &setmocktime}, {"hidden", &mockscheduler}, {"hidden", &echo},
        {"hidden", &echojson}, {"hidden", &echoipc},
#if defined(USE_SYSCALL_SANDBOX)
