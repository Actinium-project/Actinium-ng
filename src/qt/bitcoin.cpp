...
static bool InitSettings() {
  ...
  if (!gArgs.ReadSettingsFile(&errors)) {
    ...
    std::string error_translated =
        QCoreApplication::translate("bitcoin-core", error.c_str())
            .toStdString();
    InitError(
        Untranslated(strprintf("%s:\n%s\n", error, MakeUnorderedList(errors))));

    QMessageBox messagebox(
        QMessageBox::Critical, PACKAGE_NAME,
        QString::fromStdString(strprintf("%s.", error_translated)),
...
static const char* qt_argv = "Actinium-qt";
...
void BitcoinApplication::initializeResult(
  ...
  if (success) {
    ...
    // If -min option passed, start window minimized (iconified) or minimized to
    // tray
    ...
    // actinium: URIs or payment requests:
    if (paymentServer) {
  ...
  QMessageBox::critical(nullptr, tr("Runaway exception"),
                        tr("A fatal error occurred. %1 can no longer continue "
                           "safely and will quit.")
                                .arg(PACKAGE_NAME) +
                            QLatin1String("<br><br>") +
                            GUIUtil::MakeHtmlLink(message, PACKAGE_BUGREPORT));
  ::exit(EXIT_FAILURE);
}
...
int GuiMain(int argc, char* argv[]) {
  ...
  // actinium: links repeatedly have their payment requests routed to this
  // process:
  if (WalletModel::isWalletEnabled()) {
